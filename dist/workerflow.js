...
class Workerflow {
  ...
  readFunction(name) {
    ...
    fse.readJson(path.resolve(__dirname).split('/dist')[0] + '/imageconfig.json').then(packageObj => {
      if (!packageObj.address || !packageObj.addressT || !packageObj.desc) {
        // 如果图片的配置文件有一项默认为空，需要进行初始化配置
    ...
    bd.stdout.on('data', data => {
      if (data.indexOf('BuildEndJS') !== -1) {
        ...
        if (self.project !== 'addressT') {
          // 这个位置可以通过 num++, 计数的方式,  数组动态改变管道函数的参数，目前先写一个if判断
        ...
        bd.on('exit', function (code) {
          ...
          if (self.project === 'addressT' && self.num % 2 === 0) {
            // 监听事件some_event 监听项目的第二次回调触发项目
